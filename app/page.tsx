import { Button } from "@/components/ui/button"
import { FiDownload } from "react-icons/fi"

// components
import Socials from "@/components/Socials"
import Photo from "@/components/Photo";

const Home = () => {
  return (
    <section className="h-full">
      <div className="container mx-auto h-full">
        <div className="flex flex-col xl:flex-row items-center justify-between">
          {/* text */}
          <div className="text-center xl:text-left order-2 xl:order-none ">
            <span>Software Developer</span>
            <h1 className="mb-6 mt-4">
              Hi, I&apos;m <br /> <span className="text-accent">Daniel Pilant</span>
            </h1>
            <p className="max-w-[500px] mb-9 text-white">
              I’m a software engineering student, 
              software developer, and community leader. I build apps, tools, 
              and rendering engines with a focus on clean design and 
              user experience. I lead DevLev, bridging academia and 
              industry, and I’ve also taught students through the 
              national Perach program.<br /> <span className="text-accent">My goal:</span> transform complex 
              ideas into simple, impactful solutions.</p>
              {/* button and social links */}
          <div className="flex flex-col xl:flex-row items-center gap-8">
            <Button size="lg" className="uppercase flex items-center gap-2">
              <span>Download resume</span>
              <FiDownload className="text-xl"/>
            </Button>
            <div className="mb-8 xl:mb-0">
              <Socials containerStyles="flex gap-6" iconStyles="w-9 h-9 border
              border-accent rounded-full flex justify-center items-center
              text-accent text-base hover:bg-accent hover:text-primary hover:transition-all duration-500" />
            </div>
          </div>
          </div>
          
          {/* photo */}
          <div className="order-1 xl:order-none mb-8 xl:mb-8">
            <Photo />
          </div>
        </div>
      </div>
    </section>
  );
}
export default Home